...
| GitHub-style autolink literals     | Bare URL/email literals (for example `https://example.com`, `www.example.com`, `name@example.com`) are auto-linked in preview, excluding inline/fenced code.                                                             |
| GitHub-style footnotes             | Footnote references and definitions (`[^1]`) render as superscript links with a grouped footnotes section and backreferences. In-app footnote navigation is hash-storage-safe (does not overwrite URL document payload). |
